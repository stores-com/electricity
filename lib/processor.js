 ...
 * @param {function(string): string} urlBuilder Builds asset URLs with configured hashes and CDN hostname.
 * @returns {function(string): Object} Function that processes a public URL path into a file record.
 */
module.exports = (directory, options, urlBuilder) => {
     ...
     * @param {string[]} [dependencies] Collects source paths for file watching when supplied.
     * @returns {string} Processed CSS content.
     */
    function readCascadingStyleSheetsFile(filePath, dependencies) {
        let data;
        ...
        try {
            ...
            // Collect Sass dependencies for file watching.
            if (dependencies) {
                result.loadedUrls.forEach(file => {
                    if (file.protocol === 'file:') {
                        dependencies.push(url.fileURLToPath(file));
                    }
                });
            }
        }
     ...
     * Logs processing failures while retaining available source.
     * @param {string} filePath Absolute path to the JavaScript file.
     * @param {string[]} [dependencies] Collects source paths for file watching when supplied.
     * @returns {string} Processed JavaScript content.
     */
    function readJavaScriptFile(filePath, dependencies) {
        let data;

        // Snockets
        try {
            // The serving cache decides when to rebuild; discard stale Snockets output.
            delete snockets.concatCache[filePath];
        ...
        // Collect Snockets dependencies for file watching.
        if (dependencies) {
            try {
                // Keep known dependencies even when recompiling the chain fails.
                dependencies.push(...snockets.depGraph.getChain(filePath));
     ...
     * @returns {Object} Content, response metadata, optional gzip data, and dependency paths in watch mode.
     */
    return urlPath => {
        let filePath = toFilePath(urlPath);
        let extension = path.extname(filePath);
        const dependencies = options.watch?.enabled ? [filePath] : undefined;
        let data;

        if (extension === '.css') {
            data = readCascadingStyleSheetsFile(filePath, dependencies);
        } else if (extension === '.js') {
            data = readJavaScriptFile(filePath, dependencies);
        } else {
        ...
        if (dependencies) {
            file.dependencies = [...new Set(dependencies)];
        }
