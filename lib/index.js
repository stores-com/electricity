...
// A single background worker thread warms every electricity.static() instance
// in the process. It's created lazily on the first warm job and shared from
// then on, so loading the heavy transform libraries (Babel/Sass/UglifyJS)
// happens once rather than once per instance.
let warmWorker = null;
let warmWorkerFailed = false;
let nextWarmJobId = 0;

// Maps a job id to the processor whose cache should receive that job's entries.
const warmJobs = new Map();

/**
 * Returns the shared warm worker, creating it on first use. Returns null if the
 * worker can't be created, in which case warming is skipped and the
 * synchronous read remains the only path.
 */
function getWarmWorker() {
    if (warmWorker || warmWorkerFailed) {
        return warmWorker;
    }

    try {
        warmWorker = new Worker(path.join(__dirname, 'warmer.js'));
    } catch (err) {
        warmWorkerFailed = true;
        //eslint-disable-next-line no-console
        console.warn(`Electricity skipping cache warming:\n    ${err}`);
        return null;
    }

    warmWorker.on('message', ({ done, file, jobId, urlPath }) => {
        const processor = warmJobs.get(jobId);
        ...
        // Don't clobber an entry a request already populated via the
        // synchronous fallback; both produce identical content.
        if (!processor.files[urlPath]) {
            processor.files[urlPath] = reviveFile(file);
        }
    });
...
/**
 * Queues a warm job on the shared worker thread. The worker lazily reads and
 * processes every file in the directory, posting finished cache entries back so
 * the expensive first read (Sass/Babel/UglifyJS/gzip) stays off the event loop.
 * @param {string} directory
 * @param {object} options
 * @param {object} processor
 */
function startWarmer(directory, options, processor) {
    const worker = getWarmWorker();
    ...
    warmJobs.set(jobId, processor);
    worker.postMessage({ directory, jobId, options });
}
