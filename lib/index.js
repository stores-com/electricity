...
function startWarmer(directory, options, processor) {
    ...
    try {
        ...
        // options aren't structured-cloneable (e.g. a Sass importer or a Babel
        // plugin passed as a function); leave warming off for this instance and
        // rely on the synchronous read.
        warmJobs.delete(jobId);
    }
}
...
exports.static = (directory, options) => {
    ...
    // keeps processing on the main thread (it registers discovered dependencies
    // with the watcher), so warming is skipped while watching.
    if (!options.watch.enabled) {
