...
/**
 * Spawns a background worker thread that lazily reads and processes every file
 * in the directory, posting finished cache entries back to the main thread.
 * This keeps the expensive first read (Sass/Babel/UglifyJS/gzip) off the event
 * loop so requests don't stall. The worker is unref'd so it never keeps the
 * process alive, and any failure simply leaves the synchronous read in place.
...
function startWarmer(directory, options, processor) {
    ...
    try {
        worker = new Worker(path.join(__dirname, 'warmer.js'), {
    ...
    worker.on('message', ({ file, urlPath }) => {
