...
exports.static = (directory = 'public', options = {}) => {
    ...
    // Warm the cache in the background by default
    if (!options.warmup) {
        options.warmup = {
            enabled: true
        };
     ...
     */
     function removeJavaScriptFile(filePath) {
        ...
        // Each cached file records its complete dependency chain.
        for (const [urlPath, file] of Object.entries(files)) {
            if (file.dependencies.includes(absoluteFilePath)) {
    ...
    // Watch mode keeps its dependency tracking and lazy builds in this process.
    if (options.warmup.enabled && !options.watch.enabled) {
